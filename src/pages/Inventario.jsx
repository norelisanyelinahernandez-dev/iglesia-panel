Ã¯Â»Â¿import { useEffect, useState } from 'react'
...
const ESTADO_BADGE = { excelente:'badge-green', bueno:'badge-green', regular:'badge-amber', daÃƒÂ±ado:'badge-red', dado_de_baja:'badge-red' }
    ...
    <div className="page">
      <div className="page-header">
        ...
        <div style={{ display:'flex', gap:10 }}>
          <button className="btn btn-ghost" onClick={() => { setError(''); setModal('prestamo') }}>Registrar PrÃƒÂ©stamo</button>
      ...
      <div className="grid-4" style={{ marginBottom:24 }}>
        {['excelente','bueno','regular','daÃƒÂ±ado'].map(e => (
          <div key={e} className="stat-card">
            <div className="stat-value">{items.filter(i=>i.estado===e).length}</div>
            <div className="stat-label"><span className={`badge ${ESTADO_BADGE[e]}`}>{{'excelente':'Excelente','bueno':'Bueno','regular':'Regular','daÃƒÂ±ado':'DaÃƒÂ±ado'}[e] || e}</span></div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:'flex', gap:4, marginBottom:20, borderBottom:'1px solid var(--border)' }}>
        {[{id:'items',label:'Items'},{ id:'prestamos',label:`PrÃƒÂ©stamos (${prestamos.filter(p=>p.estado==='prestado').length} activos)`}].map(t => (
        ...
        <>
          <div style={{ marginBottom:16 }}>
            <div className="search-bar">
              <span className="search-icon">Ã¢ÂŒÂ•</span>
                ...
                <tbody>
                  ...
                  : filtered.map(i => (
                    <tr key={i.id}>
                      ...
                      <td style={{ color:'var(--text-muted)' }}>{categorias.find(c=>c.id===i.categoria_id)?.nombre || 'ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â'}</td>
                      <td style={{ fontWeight:600, color:'var(--blue)' }}>{i.cantidad}</td>
                      <td><span className={`badge ${ESTADO_BADGE[i.estado]}`}>{{'excelente':'Excelente','bueno':'Bueno','regular':'Regular','daÃƒÂ±ado':'DaÃƒÂ±ado','dado_de_baja':'Dado de baja'}[i.estado] || i.estado}</span></td>
                      <td style={{ color:'var(--text-muted)' }}>{i.ubicacion || 'ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â'}</td>
                      <td style={{ color:'var(--text-muted)', fontSize:12 }}>{[i.marca, i.modelo].filter(Boolean).join(' / ') || 'ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â'}</td>
                      <td>
                        <div style={{ display:'flex', gap:6 }}>
                          <button className="btn btn-ghost" style={{ padding:'5px 10px', fontSize:12 }} onClick={()=>openEdit(i)}>Editar</button>
                          <button className="btn btn-danger" style={{ padding:'5px 10px', fontSize:12 }} onClick={()=>handleDelete(i.id)}>Ã¢ÂœÂ•</button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                ...
                : prestamos.length === 0
                  ? <tr><td colSpan={6} style={{ textAlign:'center', padding:30, color:'var(--text-muted)' }}>Sin PrÃƒÂ©stamos registrados</td></tr>
                  : prestamos.map(p => (
                    <tr key={p.id}>
                      <td style={{ fontWeight:500 }}>{items.find(i=>i.id===p.item_id)?.nombre || p.item_id}</td>
                      <td>{p.nombre_externo || p.miembro_id || 'ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â'}</td>
                      <td style={{ color:'var(--text-muted)' }}>{new Date(p.fecha_salida).toLocaleDateString('es-DO')}</td>
                      <td style={{ color:'var(--text-muted)' }}>{p.fecha_retorno_esperada ? new Date(p.fecha_retorno_esperada).toLocaleDateString('es-DO') : 'ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â'}</td>
                      ...
                      <td>
                        {p.estado === 'prestado' && (
                          <button className="btn btn-ghost" style={{ padding:'5px 12px', fontSize:12 }} onClick={()=>devolverPrestamo(p.id)}>ÃƒÂ¢Ã…Â“Ã¢Â€Âœ Devolver</button>
                        )}
                      </td>
                    </tr>
                  ))}
            ...
            <div className="grid-2">
              ...
              <div className="form-group">
                ...
                <select value={form.categoria_id} onChange={e=>setForm({...form,categoria_id:e.target.value})} className="form-input" required>
                  <option value="">ÃƒÂ¢Ã¢Â‚Â¬â€” Seleccionar â€”Ã¢Â€Â</option>
              ...
              <div className="form-group">
                ...
                <select value={form.estado} onChange={e=>setForm({...form,estado:e.target.value})} className="form-input">
                  {[['excelente','Excelente'],['bueno','Bueno'],['regular','Regular'],['daÃƒÂ±ado','DaÃƒÂ±ado'],['dado_de_baja','Dado de baja']].map(([v,l])=><option key={v} value={v}>{l}</option>)}
                </select>
              </div>
              <div className="form-group">
      ...
      {/* Prestamo Modal */}
      {modal === 'prestamo' && (
        <Modal title="Registrar PrÃƒÂ©stamo" onClose={()=>setModal(null)}>
          {error && <div className="alert alert-error">{error}</div>}
          <form onSubmit={submitPrestamo} style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
              <button type="button" className="btn btn-ghost" onClick={()=>setModal(null)}>Cancelar</button>
              <button type="submit" className="btn btn-gold" disabled={saving}>{saving?'Guardando...':'Registrar PrÃƒÂ©stamo'}</button>
            </div>
          </form>
        </Modal>
      )}
